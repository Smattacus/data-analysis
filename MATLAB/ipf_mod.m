function ipf_mod(arg1,arg2,arg3,arg4)
...
global X Y xx yy xo dx NumPeaks NumTrials Shape ShapeVals AA PEAKHEIGHTS xxx 
global start user_start extra delta AUTOZERO SavedSignal logplot FIXEDPARAMETERS 
%
format short g
format compact
warning off all
...
global X Y xx yy xo dx user_start start FitResults NumPeaks NumTrials Shape residual FIXEDPARAMETERS
...
if isscalar(key),
    ...
    switch double(key),
        ...
        case 48  % Zero key is pressed to enter number of peaks
            ...
            xlabel(['Number of peaks = ' num2str(NumPeaks) '    Shape = ' ShapeString  ] )
            clear user_start
        case 33 % When 'Shift-1' (i.e. !) is pressed
            %Assign manual starting values to the peaks.
            user_start = [];
            n = max(xx)-min(xx);
            startpos=[n/(NumPeaks+1):n/(NumPeaks+1):n-(n/(NumPeaks+1))]+min(xx);
            for marker=1:NumPeaks, markx=startpos(marker);
                user_start = [user_start markx input(sprintf(...
                    'Enter desired start for peak #%d', marker))];
            end
            start = user_start;
            RedrawSignal(X,Y,xo, dx, NumPeaks);
        case {103,108,111,112,101,104,59,106,117,115,71,76,96,72,58,80,123, 77}
            % Selects peak shape when G,L,O,P,E,H,:,J,U,S, `, Shift-H, Shift-M or : key is pressed
            switch key
                ...
                case 77 %When 'Shift-M' is pressed, peak shape is set to a mix.
                    n=18;
                    display('Make sure to set the number of peaks first!');
                    display('Reminder: Push Shift-S to display numbers for shapes');
                    if isempty(NumPeaks), NumPeaks=1; end
                    ShapeVals = zeros(NumPeaks,1);
                    for i=1:NumPeaks
                        ShapeVals(i) = input(sprintf ...
                            ('Enter Number for Shape Type of Peak %i: ', i));
                    end
                otherwise
            end
            % switch
            if round(n)~=Shape,
                ...
                switch Shape
                    ...
                    case 17
                        ShapeString='Fixed-position Lorentzians';
                    case 18
                        ShapeString='User Defined Mix';
                    otherwise
                end % switch
                figure(1);subplot(2,1,2)
                xlabel(['Number of peaks = ' num2str(NumPeaks) '    Shape = ' ShapeString  ] )
            end % if
        case 83 % If 'shift S' is pressed.
            %Prints out numeric codes for Shapes.
            disp(' ')
            disp('Numeric Codes for Shapes')
            disp('1 - Gaussian')
            disp('2 - Lorentzian')
            disp('3 - Logistic')
            disp('4 - Pearson')
            disp('5 - Exponential Broadened Gaussian')
            disp('6 - Equal Width Gaussian')
            disp('7 - Equal - Width Lorentzian')
            disp('8 - Equal - Width ExpGauss')
            disp('9 - Exponential Pulse')
            disp('10 - Sigmoid')
            disp('11 - Fixed-Width Gaussian')
            disp('12 - Fixed - Width Lorentzian')
            disp('13 - Gauss / Lorentz Blend')
            disp('14 - Bifurcated Gaussian')
            disp('15 - Bifurcated Lorentzian')
            disp('16 - Fixed - Position gaussians')
            disp('17 - Fixed - Position Lorentzians')
...
if isempty(user_start)
    ...
    startpos = [];
    for i=1:NumPeaks
        startpos = start(2 * i - 1);
        plot([start(2 * i - 1) start(2 * i - 1)], [lyy uyy], 'm--');
    end
end
hold off
%
