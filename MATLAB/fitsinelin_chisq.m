function [est, model] = fitsinelin_chisq(xdata, ydata, ydata_errors, start)
...
%
%model =  A * sin ( 2 * pi * k * x + phi) + B * x + C
%
...
%Outputs:
%est        - Parameters which correspond to a local minimum found near the
...
est = fminsearch(model, start);
    function [chi_sq, FittedCurve] = fitfunc(params)
        ...
        B = params(4);
        C = params(5);
        FittedCurve = A * sin(2 * pi *  k * xdata + phi) + B * xdata + C;
